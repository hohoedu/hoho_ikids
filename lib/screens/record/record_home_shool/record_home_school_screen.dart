...
class RecordHomeSchoolScreen extends StatefulWidget {
  ...
  const RecordHomeSchoolScreen({super.key, required this.type});
...
class _RecordHomeSchoolScreenState extends State<RecordHomeSchoolScreen> {
  ...
  List<BarChartGroupData> _generateBarGroups(constraints) {
    ...
    final nonZeroScores = scores.where((score) => score != 0.0).toList();
    final maxScore = nonZeroScores.isNotEmpty
        ? nonZeroScores.reduce((a, b) => a > b ? a : b)
        : 0;
    final minScore = nonZeroScores.isNotEmpty
        ? nonZeroScores.reduce((a, b) => a < b ? a : b)
        : 0;
    ...
    return List.generate(
      ...
      (index) {
        ...
        if (score != 0) {
          if (score == maxScore) {
            barColor = Color(0xFFF86239);
          } else if (score == minScore) {
            barColor = Color(0xFF77BB28);
          }
        }
