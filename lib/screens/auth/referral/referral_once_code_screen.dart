...
class ReferralOnceCodeScreen extends StatefulWidget {
  final String id;
  final String password;
  final bool isAutoLoginChecked;

  const ReferralOnceCodeScreen({
    super.key,
    required this.id,
  ...
  @override
  State<ReferralOnceCodeScreen> createState() => _ReferralOnceCodeScreenState();
}

class _ReferralOnceCodeScreenState extends State<ReferralOnceCodeScreen> {
                ...
                AuthButton(
                  onTap: () async {
                    if (!isCode1Verified) {
                      oneButtonDialog(
                        title: '코드 등록',
                        content: '가입 코드를 입력하고 인증을 완료해 주세요.',
                        onTap: () => Get.back(),
                        buttonText: '확인',
                      );
                    } else {
