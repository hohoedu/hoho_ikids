...
class HaniStrokePath {
  final Path path;
  final String strokeClass;

  HaniStrokePath({required this.path, required this.strokeClass});
}

class HaniStrokeWord extends StatefulWidget {
  ...
  @override
  State<HaniStrokeWord> createState() => _HaniStrokeWordState();
}

class _HaniStrokeWordState extends State<HaniStrokeWord> {
  ...
  List<HaniStrokePath> remainingStrokePaths = [];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        remainingStrokePaths = allElements.where((e) => ['st1', 'st2', 'st3'].contains(e['class'])).map((e) {
          ...
          return HaniStrokePath(path: path, strokeClass: e['class']!);
        }).toList();
        _updatePointer();
      }
    } catch (e) {
