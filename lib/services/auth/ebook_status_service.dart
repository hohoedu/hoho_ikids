...
// 이북 사용 여부 확인
Future<void> ebookStatusService(id, schoolId, year) async {
  final userEbookDataController = Get.put(UserEbookDataController(), permanent: true);
  String url = dotenv.get('EBOOK_USE_URL');
    ...
    if (response.statusCode == 200) {
      ...
      if (resultValue == "000") {
        ...
        if (resultList['Hani'] == 'Y') {
          await haniListService(id, schoolId, year);
        }
        if (resultList['Booki'] == 'Y') {
          await bookiListService(id, schoolId, year);
        }
