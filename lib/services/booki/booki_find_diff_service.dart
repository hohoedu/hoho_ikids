...
import 'package:hani_booki/screens/booki/find_diff/find_diff_screen.dart';
...
Future<void> bookiFindDiffService(id, keyCode, year) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // 응답 결과가 있는 경우
      if (resultValue == "0000") {
        List<BookiFindDiffData> bookiFindDiffDataList =
            resultList.map((item) => BookiFindDiffData.fromJson(item)).toList();

        final BookiFindDiffDataController bookiFindDiffDataController =
            Get.put(BookiFindDiffDataController());
